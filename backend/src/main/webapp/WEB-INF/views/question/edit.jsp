...
<div class="container">
    <h2>编辑题目</h2>
    <form id="questionForm" action="${pageContext.request.contextPath}/question/update" method="post" novalidate>
        <input type="hidden" name="id" value="${question.id}">
        <div class="form-group">
            <label>科目ID</label>
            <input type="number" name="subjectId" value="${question.subjectId}" min="1">
        </div>
        <div class="form-group">
            <label>题型</label>
            <select name="type" id="questionType">
                ...
                <option value="3" ${question.type==3?'selected':''}>判断题</option>
                <option value="4" ${question.type==4?'selected':''}>填空题</option>
                <option value="5" ${question.type==5?'selected':''}>简答题</option>
            </select>
        </div>
        <div class="form-group">
            <label>题目内容</label>
            <textarea name="content" rows="3">${question.content}</textarea>
        </div>
        <div id="optionsGroup">
            <div class="form-group">
                <label>选项A</label>
        ...
        <div id="judgmentGroup" style="display:none;">
            <div class="form-group">
                ...
                <div class="options-inline">
                    <label><input type="radio" name="judgmentAnswer" value="T" ${question.answer=='T'?'checked':''}> 对（正确）</label>
                    <label><input type="radio" name="judgmentAnswer" value="F" ${question.answer=='F'?'checked':''}> 错（错误）</label>
                </div>
            </div>
        </div>
        <div class="form-group" id="answerGroup">
            ...
            <div id="answerMultiCheckboxes" style="display:none;">
                <label><input type="checkbox" name="answerMulti" value="A" ${question.answer.contains('A')?'checked':''}> A</label>
                <label><input type="checkbox" name="answerMulti" value="B" ${question.answer.contains('B')?'checked':''}> B</label>
                <label><input type="checkbox" name="answerMulti" value="C" ${question.answer.contains('C')?'checked':''}> C</label>
                <label><input type="checkbox" name="answerMulti" value="D" ${question.answer.contains('D')?'checked':''}> D</label>
            </div>
            <input type="text" name="answer" id="answerInput" value="${question.answer}" placeholder="单选填A/B/C/D">
        </div>
        <div class="form-group">
            <label>分值</label>
            <input type="number" name="score" value="${question.score}" min="1" max="100">
        </div>
...
document.getElementById('questionForm').onsubmit = function(e) {
    ...
    if (ok) { showToast('正在保存修改...', 'info'); this.submit(); }
};
</script>
</body>
</html>
